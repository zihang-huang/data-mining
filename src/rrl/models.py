...
class RRL:
    def __init__(self, dim_list, device, use_not=False, is_rank0=False, log_file=None, writer=None, left=None,
        ...
        if distributed:
            self.net = MyDistributedDataParallel(self.net, device_ids=[self.device])
    ...
    def detect_dead_node(self, data_loader=None):
        with torch.no_grad():
            for layer in self.net.layer_list[:-1]:
                layer.node_activation_cnt = torch.zeros(layer.output_dim, dtype=torch.float32, device=self.device)
                layer.forward_tot = 0
